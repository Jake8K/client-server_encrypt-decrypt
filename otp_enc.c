...
int main(int argc, char *argv[]) {
	...
	if (msgChars == -1) {fprintf(stderr, "%s contains invalid characters\n", argv[1]); exit(1); }
	if (keyChars == -1) {fprintf(stderr, "%s contains invalid characters\n", argv[2]); exit(1); }
	if (msgChars > keyChars) {fprintf(stderr, "insufficient cipher characters in key %s\n", argv[2]); exit(1); }



	if(DEBUG) {printf("otp_enc pid: %d\n connecting...\n", getpid());}	
	...
	if(DEBUG){ printf("otp_enc socket created\n"); }		
	
	// Connect to server
	if (connect(socketFD, (struct sockaddr*)&serverAddress, sizeof(serverAddress)) < 0) // Connect socket to address
		error("CLIENT: ERROR connecting");
	if(DEBUG){ printf("otp_enc connected\n"); }		

	//verify server ID
	verifyConnect = clientHandshake(socketFD, "otp_enc_d", "otp_enc");
	if(DEBUG) {printf("verify connect = %d\n", verifyConnect);}
	if (verifyConnect != 0) {
		if(verifyConnect == 1) {error("could not verify identity of otp_enc_d. process terminating\n");}
		exit(1);
	}
	if(DEBUG){ printf("otp_enc_d connection verified\n"); }		
	
	//send the files
	sendFile(msgFD, socketFD, msgChars);
	sendFile(keyFD, socketFD, keyChars);
	if(DEBUG){ printf("otp_enc files sent\n"); }		

	// Get return message from server
	char encryption[msgChars];
	...
	while(strstr(buffer, "@@@") == NULL){
		...
		strcat(encryption, buffer);
	}
	terminalLocation = strstr(encryption, "@@@") - encryption;
	encryption[terminalLocation] = '\0';
...
//function to verify valid connect (to enc_d)
/****************************************************
